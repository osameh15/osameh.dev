...
//
// Rate limiting runs BEFORE reCAPTCHA on purpose. It is not replaced by
// verification: it is what stops a flood from spending Google's verification
// quota, and it still applies to every request that reaches this point.
if (!rateAllowed()) jsonResponse(['success' => false, 'message' => 'Too many messages from this connection. Please try again later.'], 429);

// ---- reCAPTCHA v3: the last gate before the mail service ----
//
// Everything below fails closed. No branch of this block may fall through to
// mail(): a missing token, an unresolvable environment, a rejected token, a
// wrong action or hostname, a low score, a malformed Google response and an
// unreachable Google all stop the request here.
//
// Only a safe reason code is ever logged. The secret and the token are not
// logged, echoed, or returned in any response.
$recaptchaToken = is_string($payload['recaptchaToken'] ?? null) ? trim((string) $payload['recaptchaToken']) : '';
if ($recaptchaToken === '') {
    error_log('contact: recaptcha missing_token');
    jsonResponse(['success' => false, 'message' => 'Verification could not be completed. Please try again.', 'csrf' => issueCsrfToken(true)], 400);
}

$recaptchaConfig = recaptchaConfig();
if ($recaptchaConfig === null) {
    // Unknown host or unavailable private secret. Never fall back to another
    // environment's configuration - refuse instead.
    error_log('contact: recaptcha verification_unavailable (no configuration for this host)');
    jsonResponse(['success' => false, 'message' => 'Message could not be sent right now. Please try again shortly.', 'csrf' => issueCsrfToken(true)], 503);
}

$recaptcha = recaptchaVerifyToken($recaptchaToken, $recaptchaConfig);
if (!$recaptcha['ok']) {
    error_log('contact: recaptcha ' . $recaptcha['reason']);
    $unavailable = in_array($recaptcha['reason'], ['verification_unavailable', 'malformed_response'], true);
    jsonResponse([
        'success' => false,
        // The score, the reason and Google's own response stay server-side.
        'message' => $unavailable
            ? 'Message could not be sent right now. Please try again shortly.'
            : 'Verification could not be completed. Please try again.',
        'csrf' => issueCsrfToken(true),
    ], $unavailable ? 503 : 403);
}
