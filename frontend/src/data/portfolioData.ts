...
export const nowItems = [
  ...
  { label: "Availability", value: `${availabilityData.profiles[availabilityData.activeStatus as keyof typeof availabilityData.profiles].label}. ${availabilityData.profiles[availabilityData.activeStatus as keyof typeof availabilityData.profiles].description}` },
];

export const changelog = [
  { version: "5.3.0", title: "Vanta architecture, adjacent Notes & contact verification", items: ["Separated frontend and backend source trees and reorganized the frontend by feature, without changing any public URL or the deploy artifact", "Added Previous/Next navigation between Engineering Notes, ordered by the authoritative Notes list", "Adjacent notes are real crawlable links that still open through the shared editor-tab lifecycle without duplicating an open tab", "Added Google reCAPTCHA v3 protection to server-side contact submission, with a fresh token generated at submit time", "Added server-side success, action, hostname and score verification that fails closed and never hands an unverified message to the mail service", "Environment site keys are selected by exact hostname, and private verification secrets stay outside the repository and the build", "Preserved rate limiting, the exact origin allowlist, the strict Content-Security-Policy, and the Service Worker API network-only rule"] },
  { version: "5.2.3", title: "README asset path resolution", items: ["Fixed double URL-encoding of GitHub README asset paths, which returned HTTP 404 for filenames containing spaces or existing percent escapes", "Stopped rewriting third-party raw.githubusercontent.com assets onto this repository, so a well-formed raw URL keeps its own owner, repository and ref", "Added idempotent, segment-safe path normalization covering Unicode filenames, literal percent characters, reserved characters, malformed escapes, and encoded slashes", "Added a repository quality gate and browser regression coverage for README asset and link URLs"] },
  { version: "5.2.2", title: "SEO discovery, semantics & loading reliability", items: ["Project, Engineering Note, and Case Study cards now expose native crawlable links while preserving the IDE tab experience", "Explore my work now navigates reliably to Projects from every section deep link", "Sitemaps now contain only independently indexable documents whose routes are intended to self-canonicalize", "WebSite structured data now uses the concise Osameh Irandoust identity with osameh.dev as its alternate name", "Native card semantics, sequential headings, targeted contrast, and compositor-safe status animation improve accessibility", "Core project content now renders from local portfolio data while live GitHub metrics load as progressive enhancement", "SSR and prerendering remain intentionally deferred while Search Console gathers evidence"] },
  { version: "5.2.1", title: "Build metadata truthfulness & HSTS ownership", items: ["Build Information no longer claims a production environment while the runtime environment is still being read, and a failed metadata request now stays neutral instead of leaving an incorrect label in place", "System Health no longer reports a guessed environment before the health endpoint responds", "Strict-Transport-Security is now owned solely by the ParsPack CDN edge, removing the duplicate header that appeared alongside the edge value on error responses", "Staging and production now occupy separate document roots with separate directory-scoped deployment accounts", "Added a quality gate that fails the build if an origin HSTS directive is reintroduced into .htaccess", "Added regression coverage proving Build Information never reports production while the environment is unresolved, delayed, or failed"] },
  { version: "5.2.0", title: "Neural Cipher identity & release codenames", items: ["Rolled out the Neural Cipher visual identity across the header, favicons, Apple touch icon, PWA install icons, and the Resume Viewer", "Introduced Cyber Noir release codenames, where a codename identifies a release family and every patch inherits it", "Surfaced the release codename in the status bar, Build Information, Command Palette, System Health, and Terminal", "Refreshed the social sharing artwork and migrated every favicon and PWA icon reference to the new pack", "Made the active editor tab scroll into view on narrow screens instead of staying off-strip", "Trimmed icon authoring assets from the deployment bundle"] },
  { version: "5.1.1", title: "Editor tabs, Source Explorer & true 404", items: ["Unified the editor-tab lifecycle so projects and Engineering Notes coexist as independent tabs and selecting Home no longer closes them", "Made closing a tab activate the tab to its left and restore the section that tab belongs to", "Recovered the Source Explorer from failed or stale file requests without breaking the tree", "Corrected Service Worker response handling and kept API responses out of the cache", "Returned real HTTP 404 responses while keeping the custom IDE 404 workspace", "Fixed Recruiter Mode tour responsiveness on mobile viewports"] },
  { version: "5.1.0", title: "Interaction reliability & developer UX", items: ["Standardized header utility controls and made the availability badge label follow the active Portfolio Mood", "Consolidated the ranked search experience into one Command Palette surface with one Ctrl/Cmd + Shift + P shortcut", "Fixed modal gutter symmetry and made case-study scroll restoration deterministic by removing delayed route-scroll races and restoring the covered workspace explicitly", "Added dedicated Engineering Note and Case Study context-menu actions", "Expanded Terminal coverage for case studies, capabilities, GitHub Activity, Portfolio Mood, Accessibility, and the Command Palette", "Hardened v5 light-theme surfaces and browser regression coverage"] },
  { version: "5.0.0", title: "Portfolio capability milestone", items: ["Published Amorella Beauty as the first verifiable freelance/client case study and separated three experience-backed capability cards from client claims", "Added a five-state portfolio mood system with one config field, CLI presets, and a safe develop → staging GitHub Actions updater", "Added a persistent Accessibility Control Center with visibly effective motion, contrast, text-size and focus controls", "Upgraded the IDE search surface into ranked Universal Search and added GitHub Activity to the Explorer sequence", "Unified modal scroll locking to prevent background scroll chaining and modal jump-back behavior", "Separated staging and production CI/CD so tested artifacts deploy only after quality, E2E and Lighthouse gates pass"] },
  { version: "4.2.2", title: "Light-theme regression hardening", items: ["Hardened the Engineering Notes light-theme contrast regression selector and visibility assertion used by the browser quality suite"] },
  { version: "4.2.1", title: "Light-theme and mobile regressions", items: ["Tightened light-theme Skills Preview contrast checks", "Kept the mobile project Gallery quick-access item fully visible when active at the end of the document", "Made Engineering Notes restoration deterministic by bypassing smooth scrolling on return"] },
  { version: "4.2.0", title: "Semantic light-theme redesign", items: ["Rebuilt Light Theme around semantic canvas, surface, text, accent and border behavior across the IDE shell", "Unified light styling for projects, Notes, Recruiter Mode, 404, compare, command surfaces, modals and navigation states", "Improved hover, selected, focus and disabled-state contrast across primary interactive surfaces"] },
  { version: "4.1.1", title: "Notes navigation & scroll restoration", items: ["Engineering Note close and browser Back now restore the exact Engineering Notes anchor only after the portfolio DOM has committed, eliminating the mixed Changelog/Notes landing position", "Browser history scroll restoration is controlled by the SPA while mounted so native restoration cannot overwrite the final Notes position", "Note TOC selection stays pinned to the clicked destination during smooth scrolling and resumes live scroll-spy tracking after the jump or any manual navigation", "TOC heading lookup is scoped to the active note article and duplicate generated heading IDs are de-duplicated, preventing unstable selections", "The header PWA install action no longer wraps with the availability label and centers its icon across desktop, tablet and mobile", "Mobile and tablet note TOCs keep the active chip visible inside their horizontal scroller without moving the document"] },
  { version: "4.1.0", title: "Responsive polish & notes navigation", items: ["Audited and strengthened light-theme contrast across activity, notes, source, health, changelog and modal surfaces", "Engineering Notes now render in batches of six and the article TOC tracks the active heading while scrolling", "Returning from an article now lands reliably at the Engineering Notes index instead of an offset position", "System Health cards no longer expose a gray grid backing when the final row is incomplete", "Release graph hover is now preview-only while selecting a node updates the persistent detail panel", "Modals are capped to a scrollable 75vh viewport and project quick access becomes a mobile bottom navigation rail"] },
  { version: "4.0.0", title: "Production engineering layer", items: ["Added dynamic project/note structured data and runtime sitemap generation", "Added develop-to-staging delivery with explicit noindex protection", "Added CI quality gates, browser E2E smoke, accessibility assertions, and Lighthouse thresholds", "Upgraded diagnostics into a live privacy-safe System Health Center", "Added Markdown Engineering Notes with deep links, TOC, code copy, sharing, Terminal and Command Palette integration"] },
  { version: "3.1.1", title: "Alignment & source status polish", items: ["Aligned the RELEASE GRAPH / LIVE HISTORY intro label with the adjacent explanatory copy by removing the inherited global eyebrow top margin", "Reworked the Source Explorer footer row so language, file size, and line count are vertically centered and never clipped", "Added subtle separators between Source Explorer status values in both dark and light themes"] },
  ...
  { version: "2.2.2", title: "Release documentation", items: ["Added complete release history under docs/CHANGELOG.md", "README now shows the five latest releases and links to full history", "Added the production screenshots to docs/img", "Finalized CI/CD documentation for scoped ParsPack deployment"] },
