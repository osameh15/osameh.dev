...
import { adjacentNotes, engineeringNotes, type EngineeringNote } from "./notesData";
...
/**
 * Adjacent-note link. A real anchor, so Ctrl/Cmd-click, middle click, "Open in
 * new tab", copy-link and keyboard activation all behave natively. A plain
 * primary click is intercepted and handed to the shared Note lifecycle, which
 * owns editor tabs, history and the route.
 */
function AdjacentNoteLink({ note, direction, onOpenNote }: { note: EngineeringNote; direction: "previous" | "next"; onOpenNote: (slug: string) => void }) {
  return <a
    className={`note-adjacent-link note-adjacent-${direction}`}
    href={`/notes/${encodeURIComponent(note.slug)}`}
    data-adjacent={direction}
    aria-label={`${direction === "previous" ? "Previous" : "Next"} note: ${note.title}`}
    onClick={event => {
  ...
  >
    <span className="note-adjacent-direction">{direction === "previous" ? <><ArrowLeft size={13} /> Previous</> : <>Next <ArrowRight size={13} /></>}</span>
    <span className="note-adjacent-title">{note.title}</span>
  </a>;
}
  ...
  return <section className="note-detail" data-note-slug={note.slug} aria-labelledby="note-detail-title">
    ...
    <div className="note-reading-layout">
      ...
      <main className="note-reading-pane">
        ...
        {(previous || next) && <nav className="note-adjacent" aria-label="Engineering Notes navigation">
