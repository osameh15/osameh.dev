  ...
  return <section ref={rootRef} id={`source-${repo.name}`} className="source-explorer" aria-labelledby={`source-title-${repo.id}`}>
    ...
    <div className="source-workbench">
      <aside className="source-sidebar">
        ...
        <div className="source-tree">{treeState === "loading" ? <p className="source-state"><LoaderCircle className="spin" size={16} /> Loading repository tree…</p> : treeState === "error" ? <p className="source-state">The source tree is currently unavailable. Try again shortly.</p> : treeState === "ready" && filteredFiles.length ? <SourceTreeNode node={sourceTree} activePath={activePath} onOpen={openFile} /> : treeState === "ready" && !files.length ? <p className="source-state">This repository has no previewable source files.</p> : treeState === "ready" ? <p className="source-state">No previewable files match this filter.</p> : <p className="source-state">Source tree loads when this section approaches the viewport.</p>}</div>
      </aside>
