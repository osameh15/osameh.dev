// Canonical technology identities - plain JavaScript so the application, the
// quality gates and the test suite all match on exactly the same table.
// The typed facade lives in technology.ts; this file holds the data and logic.
//
// Two rules that are easy to get wrong:
//   - C# and .NET are related but distinct, and so are Qt and QML. A combined
//     source token such as "C# / .NET" resolves to BOTH keys.
//   - A token that is not a technology is not forced into the registry.
//     Descriptive portfolio.json concepts resolve to nothing on purpose.
 ...
 */
export function canonicalKeys(token) {
  if (!token) return [];
...
export function canonicalKeysFor(tokens) {
  const keys = [];
