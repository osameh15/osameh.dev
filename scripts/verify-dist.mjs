...
const required = [
  ...
  "dist/api/health.php", "dist/project.php", "dist/note.php", "dist/sitemap.php", "dist/notes-index.json", "dist/sw.js"
];
...
for (const ref of localRefs) {
  if (["/", "/home", "/about", "/projects", "/experience", "/now", "/changelog", "/notes", "/contact", "/resume"].includes(ref)) continue;
  const file = resolve(`dist${ref}`);
  if (!existsSync(file) && !ref.startsWith("/projects/") && !ref.startsWith("/notes/")) throw new Error(`Broken built local reference: ${ref}`);
}
console.log(`Verified deployment bundle and ${notes.length} notes.`);
