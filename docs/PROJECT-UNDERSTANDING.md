...
## 27. v5.3.3 Vanta additions

**Branded HTTP errors.** Eleven static documents at `/errors/`, declared with
`ErrorDocument` and served through an internal Apache subrequest, so the
original URL and the original status both survive. They carry the IDE identity
with an `error_<status>.cpp` editor tab, contain no script and no bundle
reference, and share one self-contained stylesheet, so they render when the
application runtime is what failed. `scripts/error-pages.mjs` is the one source
of truth; the deploy assembler writes them beside `.htaccess` and the service
worker.

**API separation.** Apache substitutes an error document only for a bodyless
response it generated itself, so application JSON is never replaced. Backend
library includes are refused by `backend/api/forbidden.php` with a JSON `403`
rather than by Apache, and every API refusal now carries a body.

**Honest coverage.** `502`, `504` and edge-level `503`/`429` are generated by the
...
1. Complete the outstanding staging acceptance pass carried since v5.2.0:
   ...
   mature 404, Source Explorer, Service Worker, and Recruiter Mode contracts.
   From v5.3.3 this also covers the branded error documents: a real document
   403 must show the portfolio's error workspace and still answer 403, and
   `/api/` must still answer JSON.
