...
export const changelog = [
  { version: "4.2.2", title: "Contrast-test reliability patch", items: ["Updated the Engineering Notes light-theme contrast regression to target the real Notes index section label", "Contrast targets now fail fast with the missing selector and active route instead of consuming the full E2E timeout"] },
  { version: "4.2.1", title: "Release-test & navigation polish", items: ["Made the Skills Preview contrast test use an exact accessible-name match so the Resume Viewer preview plugin cannot cause a Playwright strict-mode collision", "Mobile project quick access now scrolls its horizontal rail to reveal the automatically selected Gallery control at the document end", "Engineering Note return navigation now bypasses global smooth scrolling and settles at the Notes index immediately", "Tightened browser regressions around immediate Notes restoration and visibility of the active mobile Gallery control"] },
  { version: "4.2.0", title: "Cohesive light-theme redesign", items: ["Introduced a semantic light palette with durable canvas, surface, text, accent and border tokens", "Reworked Hero engineering cards so Primary lanes, metrics, terminal output and stack chips remain clearly readable", "Converted Skills Preview and code modes into intentional light surfaces with accessible interactive states", "Unified light-theme treatment across tabs, project routes, Engineering Notes, Recruiter Mode, 404, command surfaces and modals", "Redesigned Compare controls and the floating compare queue with high-contrast selected, hover and disabled states", "Added automated contrast regression coverage for representative light-theme routes and controls"] },
  { version: "4.1.2", title: "Mobile toolbar finishing pass", items: ["Rebuilt the compact PWA install control with an independently centered square icon frame", "Project quick access now treats the end of the document as the Gallery state so the final mobile toolbar item activates automatically", "Engineering Note navigation now sticks below both the fixed top bar and the editor tab row on tablet and mobile", "Added responsive regression coverage for PWA icon geometry, Gallery end-state selection, and Notes toolbar stacking"] },
  { version: "4.1.1", title: "Notes navigation stabilization", items: ["Engineering Note return navigation now waits for the portfolio DOM commit and corrects late layout shifts before settling at the Notes section", "Browser history scroll restoration is managed by the application so Back navigation cannot reintroduce the previous article offset", "Article heading IDs are namespaced and TOC lookups stay inside the active article, preventing collisions with the surrounding IDE shell", "Repeated TOC clicks keep the selected item stable during smooth scrolling while manual wheel or touch input immediately restores scroll-driven selection", "The mobile install control now inherits the full header height and centers its download icon precisely"] },
